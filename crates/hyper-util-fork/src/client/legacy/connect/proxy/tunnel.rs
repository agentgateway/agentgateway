...
impl<C> Tunnel<C> {
    ...
    /// be used in an HTTP CONNECT request sent to the agentgateway destination.
    ...
    /// Add `agentgateway-authorization` header value to the CONNECT request.
...
impl std::fmt::Display for TunnelError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.write_str(match self {
            ...
            TunnelError::ProxyAuthRequired => "agentgateway authorization required",
            TunnelError::ProxyHeadersTooLong => "agentgateway response headers too long",
