...
function normalizeLanguage(value: string | null | undefined): AppLanguage | undefined {
 ...
 */
export function tr(key: string, values?: TranslationValues | TranslationOptions): string {
	const { count, replacements } = normalizeValues(values);
	const translated =
		count === undefined ? resourceValue(key) : i18n.t(key, { count, defaultValue: key });
...
export function translateText(
	...
	const translated = resourceValue(`copy.${copyKeySegment(english)}`) ?? english;
...
function interpolateValues(translated: string, values?: TranslationValues) {
	const replacements = Array.isArray(values) ? values : values === undefined ? [] : [values];
	let index = 0;
	return translated.replace(/\{\{value\}\}/g, () => String(replacements[index++] ?? ''));
}
