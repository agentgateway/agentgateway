...
pub mod from_completions {
	...
	use crate::conversion::completions::{parse_data_url, parse_request_with_cache_control};
	...
	pub fn translate(req: &types::completions::Request) -> Result<Vec<u8>, AIError> {
		let typed = parse_request_with_cache_control(req).map_err(AIError::RequestMarshal)?;
