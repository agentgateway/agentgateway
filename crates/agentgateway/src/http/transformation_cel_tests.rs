...
fn sensitive_header_test_inputs() -> (
	Transformation,
	crate::proxy::httpproxy::PolicyClient,
	crate::http::Request,
) {
	let transformation = Transformation::try_from_local_config(
		LocalTransformationConfig {
			request: Some(LocalTransform {
				set: vec![(
					strng::literal!("authorization"),
					strng::literal!(r#"request.headers["my-mcp-token"]"#),
				)],
				..Default::default()
			}),
			response: None,
	...
	.unwrap();
	let client = crate::proxy::httpproxy::PolicyClient::new(
...
"#,
	...
	let request = ::http::Request::builder()
		...
		.header("my-mcp-token", "super-secret-token")
		.body(crate::http::Body::empty())
		.unwrap();
	(transformation, client, request)
}

fn assert_sensitive_headers_redacted(request: &crate::http::Request) {
	assert!(request.headers()["my-mcp-token"].is_sensitive());
	assert!(request.headers()["authorization"].is_sensitive());
	assert_eq!(request.headers()["authorization"], "super-secret-token");
	assert!(!format!("{request:?}").contains("super-secret-token"));
}

#[tokio::test]
async fn configured_sensitive_headers_are_redacted_after_backend_transformation() {
	let (transformation, client, mut request) = sensitive_header_test_inputs();
	let mut log = None;

	let _ = crate::store::BackendPolicyTrait::apply(&transformation, &client, &mut log, &mut request)
		.await
		.unwrap();

	assert_sensitive_headers_redacted(&request);
}

#[tokio::test]
async fn configured_sensitive_headers_are_redacted_after_request_transformation() {
	let (transformation, client, mut request) = sensitive_header_test_inputs();
	let start = agent_core::Timestamp::now();
	let mut log = crate::telemetry::log::RequestLog::new(
		crate::telemetry::log::CelLogging::new(
			client.inputs.cfg.logging.clone(),
			client.inputs.cfg.metrics.clone(),
		),
		client.inputs.metrics.clone(),
	...
	let _ = crate::store::RequestPolicyTrait::apply(&transformation, &client, &mut log, &mut request)
		.await
		.unwrap();
