...
func (in *ExtProc) DeepCopyInto(out *ExtProc) {
	*out = *in
	...
	if in.MetadataContext != nil {
		...
		*out = make(map[string]map[string]shared.CELExpression, len(*in))
		for key, val := range *in {
			var outInner map[string]shared.CELExpression
			if val != nil {
				outInner = make(map[string]shared.CELExpression, len(val))
				for k, v := range val {
					outInner[k] = v
				}
			}
