use crate::crypto::digest::{sha256, sha512};
use crate::http::aauth::sig::encoding::base64_encode;
...
pub fn calculate_content_digest(body: &[u8], algorithm: DigestAlgorithm) -> String {
	let hash_bytes: Vec<u8> = match algorithm {
		DigestAlgorithm::Sha256 => sha256(body).to_vec(),
		DigestAlgorithm::Sha512 => sha512(body).to_vec(),
	};
