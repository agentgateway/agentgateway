...
// the Gateway). Attachment is sourced from `spec.targetRefs` (the user's
// declarative intent) rather than `policy.status.ancestors` (which the
// translator writes downstream and is therefore not authoritative for the
// deployer's decision).
	...
	return krt.NewCollection(gateways, func(kctx krt.HandlerContext, gw *gwv1.Gateway) *OIDCRequiredGateway {
		...
		anyRouteAttachesOIDC := func(parent gatewayObjectKey) bool {
			for _, rt := range krt.Fetch(kctx, httpRoutes, krt.FilterIndex(httpRoutesByParent, parent)) {
				if hasAttachedOIDC(wellknown.HTTPRouteKind, rt.Namespace, rt.Name) {
					return true
				}
			}
			for _, rt := range krt.Fetch(kctx, grpcRoutes, krt.FilterIndex(grpcRoutesByParent, parent)) {
				if hasAttachedOIDC(wellknown.GRPCRouteKind, rt.Namespace, rt.Name) {
					return true
				}
			}
			return false
		...
		if hasAttachedOIDC(wellknown.GatewayKind, gw.Namespace, gw.Name) {
			return &gwKey
		}
		...
		for _, ls := range krt.Fetch(kctx, listenerSets, krt.FilterIndex(listenerSetsByParentGateway, collections.TargetRefIndexKey{
			...
			if hasAttachedOIDC(wellknown.ListenerSetKind, ls.Namespace, ls.Name) {
				return &gwKey
			}
			if anyRouteAttachesOIDC(gatewayObjectKey{
