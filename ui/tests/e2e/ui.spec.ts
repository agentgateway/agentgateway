...
test("LLM playground forwards its selected key to MCP requests", async ({
  page,
}) => {
  const gateway = await mockGateway(page);
  const bearerToken = "llm-mcp-secret";

  await page.route("**/mcp", async (route) => {
    if (route.request().headers().authorization === `Bearer ${bearerToken}`) {
      await route.fallback();
      return;
    }
    await route.fulfill({
  ...
  await page.route("**/v1/chat/completions", async (route) => {
    ...
    await route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify({
        id: "chatcmpl-mcp-auth-test",
        choices: [
          hasToolResult
            ? {
                message: { role: "assistant", content: "tool flow complete" },
                finish_reason: "stop",
              }
            : {
                message: {
  ...
  await page.getByRole("combobox", { name: "Model" }).click();
  await page.getByRole("option", { name: /resilient/ }).click();
  await page.getByRole("combobox", { name: "Virtual API key" }).click();
  await page.getByRole("option", { name: "Raw value" }).click();
  await page.getByLabel("Raw API key").fill(bearerToken);
  await page.getByRole("checkbox", { name: /Include MCP tools/ }).check();
  await page.getByLabel("User message").fill("use the echo tool");
  await page.getByRole("button", { name: "Send" }).click();

  await expect(
    page.locator(".chat-message.assistant .chat-bubble").last(),
  ).toContainText("tool flow complete");
  const expectedMcpMethods = [
    "initialize",
    "tools/list",
    "tools/call",
  ];
  await expect
    .poll(() => gateway.mcpRequests.map((request) => request.method))
    .toEqual(expect.arrayContaining(expectedMcpMethods));
  expect(
