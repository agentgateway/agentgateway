...
| `traced`           | Top-level wrapper that evaluates the inner expression unchanged, logs the expression and result at info on success, and logs at warn on error. Example: `traced(request.headers["b"] + "c")`                                                                                   |
| `tracedErrors`     | Top-level wrapper that evaluates the inner expression unchanged and only logs at warn when evaluation fails. Example: `tracedErrors(request.headers["b"] + "c")`                                                                                                               |
