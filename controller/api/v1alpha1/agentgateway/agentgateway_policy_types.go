...
// +kubebuilder:validation:XValidation:rule="has(self.headers) || has(self.key) || has(self.secretRef) || has(self.passthrough) || has(self.aws) || has(self.azure) || has(self.gcp)",message="must specify headers or exactly one of key/secretRef/passthrough/aws/azure/gcp"
