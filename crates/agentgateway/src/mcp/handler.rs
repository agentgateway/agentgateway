			...
			let tools = streams
				...
				.flat_map(|(server_name, s)| {
					...
					tools
						...
						// SEP-2243: a client MUST exclude tools whose x-mcp-header annotations are invalid from
						// tools/list. The gateway is a client toward its upstream, so it drops them here.
						.filter(|t| {
							match crate::mcp::param_validation::validate_x_mcp_header_annotations(&t.input_schema)
							{
								Ok(()) => true,
								Err(e) => {
