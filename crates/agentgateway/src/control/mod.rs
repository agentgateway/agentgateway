...
mod tests {
	...
	#[tokio::test]
	async fn grpc_connector_adds_custom_headers_to_requests() {
		...
		tokio::spawn(async move {
			...
			let service = hyper::service::service_fn(move |request| {
				...
				async move {
					if let Some(tx) = headers_tx.lock().expect("mutex poisoned").take() {
						tx.send(request.headers().clone())
							.expect("test should receive headers");
					}
		...
		let mut connector = grpc_connector(
			client,
			format!("http://{address}"),
			AuthSource::StaticToken(SecretString::new("xds-token".into()), "cluster-a".into()),
			RootCert::Default,
			vec![
				(
					"x-istio-revision".parse().unwrap(),
					"canary".parse().unwrap(),
				),
				("x-tenant".parse().unwrap(), "team-a".parse().unwrap()),
				("authorization".parse().unwrap(), "custom".parse().unwrap()),
			],
		)
		.await
		...
		connector
			.call(
				::http::Request::builder()
					...
					.uri(
						"http://xds.test/agent.discovery.v1.AggregatedDiscoveryService/StreamAggregatedResources",
					)
					.body(Body::empty())
					.unwrap(),
			)
			.await
			.expect("xDS request should succeed");

		let headers = tokio::time::timeout(std::time::Duration::from_secs(5), headers_rx)
			.await
			.expect("timed out waiting for xDS request")
			.expect("server should send request headers");
		assert_eq!(headers["x-istio-revision"], "canary");
		assert_eq!(headers["x-tenant"], "team-a");
		assert_eq!(headers["authorization"], "Bearer xds-token");
		assert_eq!(headers["clusterid"], "cluster-a");
	}
}
