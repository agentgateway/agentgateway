...
export function schemaHelpYaml(
	...
	return schemaNodeLines(resolved, help, t, 0, { maxDepth: 4 }).join('\n').trim();
}
...
function schemaNodeLines(
	...
	for (const [key, child] of entries) {
		...
		const childDescription = localizeDescription(child.description ?? childResolved.description);
		if (childDescription) pushComment(lines, childDescription, indent);
		const meta = [schemaTypeLabel(childResolved, t), required.has(key) ? t('schema.required') : '']
			.filter(Boolean)
			.join(', ');
