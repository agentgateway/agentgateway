    ...
    <FieldGroup label={props.label} tooltip={props.tooltip}>
      <div className="list-editor">
          ...
          <div className="chip-list">
            {props.values.map((value) => (
              <span className="config-chip" key={value}>
                <span>{value}</span>
                <button type="button" aria-label={`Remove ${value}`} onClick={() => props.onChange(props.values.filter((item) => item !== value))}>x</button>
              </span>
            ))}
          </div>
        ...
        <div className="list-editor-row">
          ...
          />
          <button className="button" type="button" onClick={() => add(draft)}>Add</button>
          {props.actions}
        </div>
        {props.suggestions?.length ? (
          <div className="suggestion-row">
            {props.suggestions.map((suggestion) => (
              <button className="table-action" type="button" key={suggestion} onClick={() => add(suggestion)}>
                {suggestion}
              </button>
            ))}
          </div>
