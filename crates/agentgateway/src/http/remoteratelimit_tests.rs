...
/// Helper: build a `DescriptorEntry` with a limit override expression.
fn make_descriptor_entry_with_limit_override(
	entries: Vec<(&str, &str)>,
	...
	DescriptorEntry {
		entries: Arc::new(descriptors),
		limit_type,
		limit_override: Some(LimitOverride {
			expression: Arc::new(cel::Expression::new_strict(expr).expect("valid CEL expression")),
		}),
	}
}
...
/// When a descriptor has a limit_override and the metadata contains valid
/// rate limit values, `build_request` should include the RateLimitOverride.
#[test]
fn build_request_limit_override_from_extproc_metadata() {
	let entry = make_descriptor_entry_with_limit_override(
	...
	// Insert ext-proc metadata with rate limit override values
	...
	assert!(
		result.is_some(),
		"expected Some when metadata contains valid rate limit"
	);
	let request = result.unwrap();
	assert_eq!(request.descriptors.len(), 1);

	let limit = request.descriptors[0].limit.as_ref();
	assert!(limit.is_some(), "expected limit override to be set");
	let limit = limit.unwrap();
	assert_eq!(limit.requests_per_unit, 50);
	assert_eq!(limit.unit, proto::RateLimitUnit::Second as i32);
}

/// When metadata does not contain the expected path, no override is sent.
#[test]
fn build_request_limit_override_missing_metadata() {
	let entry = make_descriptor_entry_with_limit_override(
		vec![("user", r#""test-user""#)],
		RateLimitType::Requests,
		"extproc.rate_limit",
	...
	// No ext-proc metadata inserted

	let result = rl.build_request(&req, RateLimitType::Requests, None);
	assert!(
		result.is_some(),
		"request should still succeed without metadata"
	);
	let request = result.unwrap();
	assert_eq!(request.descriptors.len(), 1);

	// No limit override should be set
	assert!(
		request.descriptors[0].limit.is_none(),
		"expected no limit override when metadata is missing"
	);
}

/// When limit_override uses a nested path like `extproc.tenant.limit`, it should
/// correctly traverse the metadata structure.
#[test]
fn build_request_limit_override_nested_path() {
	let entry = make_descriptor_entry_with_limit_override(
		vec![("user", r#""test-user""#)],
		RateLimitType::Requests,
		"extproc.tenant.limit",
	);
	let rl = make_rate_limit(vec![entry]);
	...
	// Insert nested ext-proc metadata
	let mut metadata = ExtProcDynamicMetadata::default();
	metadata.0.insert(
		"tenant".to_string(),
		serde_json::json!({
	...
	let limit = request.descriptors[0].limit.as_ref();
	assert!(limit.is_some(), "expected nested limit override to be set");
	let limit = limit.unwrap();
	assert_eq!(limit.requests_per_unit, 100);
	assert_eq!(limit.unit, proto::RateLimitUnit::Minute as i32);
}

/// Test all supported unit string conversions.
#[test]
fn build_request_limit_override_unit_mapping() {
	for (unit_str, expected_unit) in [
		...
		let result = rl.build_request(&req, RateLimitType::Requests, None);
		assert!(result.is_some(), "expected Some for unit {}", unit_str);
		let request = result.unwrap();
		let limit = request.descriptors[0].limit.as_ref().unwrap();
		assert_eq!(
			limit.unit, expected_unit as i32,
			"unit {} should map to {:?}",
...
/// Backward compatibility: when no limit_override is configured, no override is sent.
#[test]
fn build_request_no_limit_override_configured() {
	// Use the regular helper without limit_override
	let entry = make_descriptor_entry(vec![("user", r#""test-user""#)], RateLimitType::Requests);
	let rl = make_rate_limit(vec![entry]);
	...
	// Even if metadata exists, no override should be sent without config
	let mut metadata = ExtProcDynamicMetadata::default();
	metadata.0.insert(
		"rate_limit".to_string(),
		serde_json::json!({
	...
	assert!(
		request.descriptors[0].limit.is_none(),
		"no limit_override config means no override sent"
	);
}

/// When the unit value is numeric (enum value), it should be accepted.
#[test]
fn build_request_limit_override_numeric_unit() {
	let entry = make_descriptor_entry_with_limit_override(
		vec![("user", r#""test-user""#)],
		RateLimitType::Requests,
		"extproc.rate_limit",
	...
	let request = result.unwrap();
	let limit = request.descriptors[0].limit.as_ref().unwrap();
	assert_eq!(limit.requests_per_unit, 25);
	assert_eq!(limit.unit, proto::RateLimitUnit::Minute as i32);
}

/// When requests_per_unit is missing, no override is sent.
#[test]
fn build_request_limit_override_missing_requests_per_unit() {
	let entry = make_descriptor_entry_with_limit_override(
		vec![("user", r#""test-user""#)],
		RateLimitType::Requests,
		"extproc.rate_limit",
	...
	assert!(result.is_some());
	assert!(
		result.unwrap().descriptors[0].limit.is_none(),
		"missing requests_per_unit should result in no override"
	);
}

/// When unit is missing, no override is sent.
#[test]
fn build_request_limit_override_missing_unit() {
	let entry = make_descriptor_entry_with_limit_override(
		vec![("user", r#""test-user""#)],
		RateLimitType::Requests,
		"extproc.rate_limit",
	...
	assert!(
		result.unwrap().descriptors[0].limit.is_none(),
		"missing unit should result in no override"
	);
}

/// Multiple rateLimits entries, each with its own limit_override pointing to different metadata paths.
/// This simulates the user config:
/// ```yaml
/// rateLimits:
/// - actions: [{descriptorKey: tier}]
///   limit:
///     dynamicMetadata: {metadataKey: {key: extproc, path: [{key: tier_limit}]}}
/// - actions: [{descriptorKey: api}]
///   limit:
///     dynamicMetadata: {metadataKey: {key: extproc, path: [{key: api_limit}]}}
/// ```
#[test]
fn build_request_multiple_rate_limits_with_different_overrides() {
	// Create two descriptors, each with different limit_override expressions
	let tier_entry = make_descriptor_entry_with_limit_override(
		vec![("tier", r#""premium""#)],
		RateLimitType::Requests,
		"extproc.tier_limit",
	);
	let api_entry = make_descriptor_entry_with_limit_override(
		vec![("api", r#""v1""#)],
		RateLimitType::Requests,
		"extproc.api_limit",
	);
	let rl = make_rate_limit(vec![tier_entry, api_entry]);
	...
	// Insert ext-proc metadata with both limit overrides
	let mut metadata = ExtProcDynamicMetadata::default();
	metadata.0.insert(
		"tier_limit".to_string(),
		serde_json::json!({
			"requests_per_unit": 100,
			"unit": "MINUTE"
		}),
	...
	// Should have 2 descriptors, each with its own limit override
	assert_eq!(request.descriptors.len(), 2);

	// First descriptor: tier_limit (100/MINUTE)
	let tier_limit = request.descriptors[0].limit.as_ref();
	assert!(
		tier_limit.is_some(),
		"tier descriptor should have limit override"
	);
	let tier_limit = tier_limit.unwrap();
	assert_eq!(tier_limit.requests_per_unit, 100);
	assert_eq!(tier_limit.unit, proto::RateLimitUnit::Minute as i32);

	// Second descriptor: api_limit (1000/HOUR)
	let api_limit = request.descriptors[1].limit.as_ref();
	assert!(
		api_limit.is_some(),
		"api descriptor should have limit override"
	);
	let api_limit = api_limit.unwrap();
	assert_eq!(api_limit.requests_per_unit, 1000);
	assert_eq!(api_limit.unit, proto::RateLimitUnit::Hour as i32);
}

/// Multiple rateLimits where only some have limit overrides in metadata.
/// Descriptors without matching metadata should still be sent, just without override.
#[test]
fn build_request_multiple_rate_limits_partial_overrides() {
	let tier_entry = make_descriptor_entry_with_limit_override(
		vec![("tier", r#""standard""#)],
		RateLimitType::Requests,
		"extproc.tier_limit",
	...
	let api_entry = make_descriptor_entry_with_limit_override(
		vec![("api", r#""v2""#)],
		RateLimitType::Requests,
		"extproc.api_limit", // This won't be in metadata
	);
	let rl = make_rate_limit(vec![tier_entry, api_entry]);
	...
	// Only insert tier_limit, not api_limit
	let mut metadata = ExtProcDynamicMetadata::default();
	metadata.0.insert(
		"tier_limit".to_string(),
		serde_json::json!({
	...
	// First descriptor has override
	let tier_limit = request.descriptors[0].limit.as_ref();
	assert!(tier_limit.is_some());
	assert_eq!(tier_limit.unwrap().requests_per_unit, 50);

	// Second descriptor has no override (metadata path not found)
	assert!(
		request.descriptors[1].limit.is_none(),
		"api descriptor should have no override when metadata missing"
	);
}

/// Mix of descriptors with and without limit_override config.
#[test]
fn build_request_multiple_rate_limits_mixed_config() {
	// First descriptor has limit_override configured
	let with_override = make_descriptor_entry_with_limit_override(
		vec![("tier", r#""gold""#)],
		RateLimitType::Requests,
		"extproc.tier_limit",
	);
	// Second descriptor has NO limit_override configured (uses static RLS limits)
	let without_override =
		make_descriptor_entry(vec![("region", r#""us-east""#)], RateLimitType::Requests);
	let rl = make_rate_limit(vec![with_override, without_override]);
	...
	// First descriptor has override from metadata
	let tier_limit = request.descriptors[0].limit.as_ref();
	assert!(tier_limit.is_some());
	assert_eq!(tier_limit.unwrap().requests_per_unit, 200);

	// Second descriptor has no override (not configured)
	assert!(
		request.descriptors[1].limit.is_none(),
		"descriptor without limit_override config should have no override"
	);
}
