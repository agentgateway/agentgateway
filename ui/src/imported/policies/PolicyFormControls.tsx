  ...
  return (
    <div className="policy-form-section">
      <div className="policy-form-section-header">
        <span className="policy-form-section-icon"><Plus size={17} /></span>
        <div>
          <h4>Target</h4>
      ...
      <div className="policy-form-section-body">
        <Field label="Host" tooltip={props.tooltip}>
          <input value={host} onChange={(event) => props.onChange({ host: event.target.value })} placeholder="host:port or unix:/path/to/socket" />
        </Field>
      </div>
    </div>
  );
    ...
    <FieldGroup label={props.label} tooltip={props.tooltip}>
      <div className="kv-editor">
          ...
          <div className="kv-list">
            {entries.map(([key, value]) => (
              <div className="kv-row" key={key}>
                <code>{key}</code>
                <span>{value}</span>
                <button className="table-action danger" type="button" onClick={() => {
        ...
        <div className="kv-add-row">
          <input value={keyDraft} onChange={(event) => setKeyDraft(event.target.value)} placeholder={props.keyPlaceholder ?? "name"} />
          <input
            ...
            placeholder={props.valuePlaceholder ?? (props.valueKind === "cel" ? "request.path" : "value")}
          />
...
export function targetFrom(value: unknown, fallbackHost = "127.0.0.1:9000"): TargetDraft {
  if (value && typeof value === "object" && !Array.isArray(value)) {
    if ("host" in value && typeof (value as { host?: unknown }).host === "string") return { host: (value as { host: string }).host };
  }
