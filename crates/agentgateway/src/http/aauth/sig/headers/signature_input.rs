...
pub struct SignatureParams {
	pub created: u64,
	// The production verifier reconstructs the `@signature-params` line from the raw Signature-Input
	// bytes (see `build_signature_base_raw`), so it never reads these structured optional params.
	// They are parsed for completeness and consumed by the test-only signer and the cross-impl
	// wire-compat vectors (e.g. the `keyid` case), hence dead only in non-test builds.
