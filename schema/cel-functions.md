...
| `removeKeys`       | Returns a new map with entries removed where the key matches the predicate. Example: `{"a":1,"b":2}.removeKeys(k, k == "b")` results in `{"a":1}`.                                                                                                                              |
| `filterKeys`       | Returns a new map keeping only entries where the key matches the predicate. Example: `{"a":1,"b":2}.filterKeys(k, k == "a")` results in `{"a":1}`.                                                                                                                              |
