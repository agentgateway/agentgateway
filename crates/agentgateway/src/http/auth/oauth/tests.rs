...
#[rstest]
#[case::matching(vec!["read".into()], None)]
#[case::empty(vec![], Some(json!([])))]
#[case::different(vec!["backend.read".into()], Some(json!(["backend.read"])))]
fn serializes_cross_app_access_scope_override(
	#[case] access_token_scopes: Vec<String>,
	...
	config.access_token_scopes = Some(access_token_scopes);
...
#[case::absent(None, &["read"])]
#[case::empty(
	Some(proto::cross_app_access_auth::ScopeList { values: vec![] }),
	&[]
)]
#[case::override_scopes(
	Some(proto::cross_app_access_auth::ScopeList {
		values: vec!["backend.read".into()],
	}),
	&["backend.read"]
)]
fn cross_app_access_from_proto_resolves_access_token_scopes(
	#[case] access_token_scopes: Option<proto::cross_app_access_auth::ScopeList>,
	#[case] expected: &[&str],
) {
	let auth = CrossAppAccessAuth::from_proto(
		proto::CrossAppAccessAuth {
