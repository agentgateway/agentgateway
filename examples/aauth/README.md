...
Build the gateway first (from the repo root); the examples below invoke the resulting
binary directly:

```sh
cargo build --bin agentgateway
```

To launch the gateway with the production-shaped example:

```sh
./target/debug/agentgateway -f examples/aauth/config.yaml
```

If you have agentgateway installed on your `PATH`, `agentgateway -f …` works too.
...
```sh
./target/debug/agentgateway -f examples/aauth/config.dev.yaml
```

This binds `127.0.0.1:3000`, routes `/echo` to `127.0.0.1:9099`, and enforces
`requiredScheme: agentJwt`. It also binds the fixed admin/stats/readiness ports — see
[Ports](#ports) if startup fails with `Address already in use`.

Unsigned requests should now return 401:
...
> **Signatures expire.** `created` is checked against a freshness window, so a captured
> set of headers stops verifying after roughly a minute. If a hand-replayed `curl` returns
> `{"error":"invalid_signature","error_description":"signature created timestamp outside
> valid window"}`, the signature simply aged out — re-capture and resend promptly rather
> than debugging the gateway. Mint fresh headers for each experiment below.
...
### 6. Confirm the `requiredScheme` ladder is enforced

A *valid* signature is not sufficient on its own — the scheme must be strong enough for
the route. With the dev config still on `requiredScheme: agentJwt`, a correctly signed
`hwk` request is rejected, and the response advertises what the route requires:

```sh
./sign-request -authority 127.0.0.1:3000 -path /echo -method GET \
  | sed 's/curl -si/curl -s -D - -o \/dev\/null/' | bash
# HTTP/1.1 401 Unauthorized
# aauth-requirement: requirement=agent-token
```

Now lower the bar — set `requiredScheme: hwk` in `config.dev.yaml`, restart the gateway,
and the *same* scheme is accepted:

```sh
./sign-request -authority 127.0.0.1:3000 -path /echo -method GET | bash
# HTTP/1.1 200 OK
```

That pair is the useful check: identical crypto, opposite outcomes, decided only by the
policy tier.

### 7. Confirm verified claims reach CEL

The `aauth.*` values in the table above are real request state, not just log output. Add
an authorization rule to the `/echo` route in `config.dev.yaml` that pins the agent
identity:

```yaml
              authorization:
                rules:
                  - 'aauth.agent == "http://127.0.0.1:9099"'
```

Restart the gateway and rerun the step-4 `agent-jwt` command — it still returns `200`.
Then change the rule to any other value (e.g. `http://other.example`), restart, and rerun
...
Same signed request, allowed or denied purely by the verified `aauth.agent` claim.
...
Beyond the listener port, the gateway also binds fixed management ports — `15000`
(admin), `15020` (stats), and `15021` (readiness). Two gateways therefore cannot run
concurrently: starting a second one fails with `Error: readiness server starts / Address
already in use (os error 48)`. When comparing configs (as in steps 6 and 7), stop the
previous instance first rather than launching them side by side.
