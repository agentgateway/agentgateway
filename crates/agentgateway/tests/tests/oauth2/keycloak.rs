...
static KEYCLOAK_LOGIN_FORM_RE: Lazy<Regex> = Lazy::new(|| {
	Regex::new(r#"(?s)<form[^>]*id=\"kc-form-login\"[^>]*action=\"([^\"]+)\""#)
		.expect("keycloak login form regex must compile")
});
...
impl KeycloakEnv {
	...
	async fn perform_browser_login_and_get_callback(
		...
		if !(login_resp.status() == StatusCode::FOUND
			|| login_resp.status() == StatusCode::SEE_OTHER
			|| login_resp.status() == StatusCode::TEMPORARY_REDIRECT)
		{
...
fn extract_keycloak_login_action(html: &str) -> Option<String> {
	let caps = KEYCLOAK_LOGIN_FORM_RE.captures(html)?;
	let action = caps.get(1)?.as_str().replace("&amp;", "&");
	Some(action)
}
