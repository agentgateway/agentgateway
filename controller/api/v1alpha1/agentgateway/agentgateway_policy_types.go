...
// +kubebuilder:validation:XValidation:rule="has(self.targetRefs) && self.targetRefs.exists(t, has(t.port)) ? has(self.frontend) : true",message="port may only be set on frontend policies"
// +kubebuilder:validation:XValidation:rule="has(self.targetSelectors) && self.targetSelectors.exists(t, has(t.port)) ? has(self.frontend) : true",message="port may only be set on frontend policies"
