...
const formats: ProviderFormat[] = ["completions", "messages", "responses", "embeddings", "anthropicTokenCount", "realtime", "rerank"];
...
export function CustomFormats(props: {
  ...
  const custom = customProvider(props.model.provider ?? "custom");

  function toggle(type: ProviderFormat, checked: boolean) {
    props.setModel((current) => {
      const currentCustom = customProvider(current.provider ?? "custom");
      const nextFormats = checked
        ? [...currentCustom.formats, { type }]
        : currentCustom.formats.filter((format) => format.type !== type);
      return { ...current, provider: { custom: { ...currentCustom, formats: nextFormats } } };
    });
  }

  function setPath(type: ProviderFormat, path: string) {
    props.setModel((current) => {
      if (!current.provider || typeof current.provider === "string" || !("custom" in current.provider)) return current;
      return {
        ...current,
        provider: {
          custom: {
            ...current.provider.custom,
            formats: current.provider.custom.formats.map((format) =>
  ...
  return (
    <div className="format-grid">
      {formats.map((type) => {
        const selected = custom.formats.find((format) => format.type === type);
        return (
          <div className="format-row" key={type}>
            <label className={selected ? "format-toggle selected" : "format-toggle"}>
              <input type="checkbox" checked={Boolean(selected)} onChange={(event) => toggle(type, event.target.checked)} />
            ...
            <input
              ...
              disabled={!selected}
              value={selected?.path ?? ""}
              placeholder={props.help.description(["$defs", "ProviderFormatConfig", "properties", "path"], "optional path override")}
