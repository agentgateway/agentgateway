import { sendMcpJsonRpc } from "./_adapters/playgroundApi";
...
export async function initializeMcpSession(baseUrl: string, clientName: string, existingSessionId?: string) {
  ...
  await sendInitializedNotification(baseUrl, sessionId);
  return sessionId;
}

export async function sendInitializedNotification(baseUrl: string, sessionId?: string | null) {
  if (!sessionId) return;
