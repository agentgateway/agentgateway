...
pub mod from_messages {
	...
	/// `ContentFilter` (`end_turn` buffered, `refusal` streamed), so the same upstream response
	/// reported a different `stop_reason` depending only on whether the client streamed. Keeping
	/// one mapping makes that class of divergence unrepresentable, including the
	/// `may_have_stop_sequence` half: only a natural `stop` can carry a matched stop sequence.
