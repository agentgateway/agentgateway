...
/// MCP authorization attached to the target (not the route) must be enforced.
#[tokio::test]
async fn target_level_authorization_denies_tool() {
	let mock = mock_streamable_http_server(true).await;
	...
	// No route-level policies; the authorization policy rides the target's backend.
	...
	let result = client
		...
		.await;
	let err = result.expect_err("target-level authorization should deny the tool call");
	...
	// tools/list filtering must apply the target-level policy too.
	let tools = client.list_tools(Default::default()).await.unwrap();
	assert!(
		tools.tools.is_empty(),
		"target-level deny-all should filter all tools, got {:?}",
		tools.tools
	);
}
