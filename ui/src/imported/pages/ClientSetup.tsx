...
import { Dropdown, Field, FieldGroup, PageHeader, Panel, StatusBanner } from "../components/Primitives";
...
type ClientRecipe = {
  ...
  title: string;
  description: string;
  icon: "claude" | "codex" | "curl" | "cursor" | "copilot" | "windsurf";
...
export function ClientSetupPage() {
  ...
  const modelOptions = useMemo(() => llmModelOptions(config.data?.llm), [config.data]);
  const virtualKeys = useMemo(() => config.data?.llm?.policies?.apiKey?.keys ?? [], [config.data]);
  ...
  const [selectedIntegration, setSelectedIntegration] = useState("claude-code");

  const selectedModel = modelOptions.some((item) => item.name === model) ? model : modelOptions[0]?.name ?? "";
  const selectedModelOption = modelOptions.find((item) => item.name === selectedModel);
  const selectedVirtualKey = virtualKeys.find((item) => item.key === selectedKey) ?? virtualKeys[0];
  const apiKey = selectedVirtualKey?.key ?? rawKey;
  const effectiveBaseUrl = baseUrlTouched ? baseUrl : derivedBaseUrl;
  const requestModel = resolveLlmModelOption(selectedModelOption, "", config.data?.llm?.providers ?? []);
  const recipes = clientRecipes({
    baseUrl: effectiveBaseUrl,
    model: requestModel || "openai/gpt-5.4-nano",
  ...
  return (
    <div className="page-stack">
      ...
      {modelOptions.length === 0 && !config.isLoading ? <StatusBanner state="warn" title="No models configured">Create an LLM model before wiring clients to the gateway.</StatusBanner> : null}
        ...
        <Panel className="client-setup-controls">
          ...
          <FieldGroup label="Virtual API key">
            <Dropdown
              ...
              value={selectedVirtualKey?.key ?? "__raw__"}
              options={[
                ...
                { value: "__raw__", label: "Raw value", icon: <Code2 size={16} /> },
              ]}
              onChange={(value) => setSelectedKey(value === "__raw__" ? "" : value)}
            />
          </FieldGroup>
          {!selectedVirtualKey ? (
            <Field label="Raw API key">
              <input value={rawKey} onChange={(event) => setRawKey(event.target.value)} placeholder="agw_sk_..." />
            </Field>
          ) : null}
          <div className="client-setup-summary">
            ...
            <div>
              <span>Auth</span>
              <code>Authorization: Bearer {apiKey ? maskKey(apiKey) : "..."}</code>
            </div>
          </div>
        </Panel>
  ...
  return [
    {
      id: "claude-code",
      title: "Claude Code",
      description: "Use the gateway URL and key with Claude-compatible model routes when configured.",
      icon: "claude",
      language: "bash",
...
claude --model "${args.model}"`,
    },
    {
      ...
      icon: "claude",
      steps: [
        <>Open Claude Desktop and enable developer mode: <strong>Help</strong> &gt; <strong>Troubleshooting</strong> &gt; <strong>Enable Developer Mode</strong>.</>,
        <>Fully quit and relaunch Claude Desktop. A new <strong>Developer</strong> menu appears in the menu bar.</>,
        <>Open <strong>Developer</strong> &gt; <strong>Configure Third-Party Inference</strong> &gt; <strong>Gateway</strong>.</>,
...
API Key: ${args.apiKey}`,
    },
    {
      id: "codex",
      title: "Codex CLI",
      description: "Use OpenAI-compatible environment variables when running Codex against the gateway.",
      icon: "codex",
      language: "bash",
...
codex --model "${args.model}"`,
    },
    {
      ...
      steps: [
        ...
        <>Add <code>{args.model}</code> as a custom model, then test from <strong>Ask</strong> or <strong>Plan</strong> mode.</>,
      ],
      language: "text",
...
const tsRules: CodeRule[] = [
  ...
  { className: "code-keyword", pattern: /\b(?:await|const|from|import|new)\b/y },
  { className: "code-number", pattern: /\b\d+(?:\.\d+)?\b/y },
  { className: "code-property", pattern: /\b(?:apiKey|baseURL|client|content|messages|model|response|role)\b(?=\s*:|\.)/y },
...
const pythonRules: CodeRule[] = [
  { className: "code-comment", pattern: /#.*/y },
  { className: "code-string", pattern: /"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'/y },
  { className: "code-keyword", pattern: /\b(?:from|import|client|response)\b/y },
  { className: "code-number", pattern: /\b\d+(?:\.\d+)?\b/y },
  { className: "code-property", pattern: /\b(?:api_key|base_url|messages|model)\b(?=\s*=)/y },
  { className: "code-function", pattern: /\b[A-Za-z_][\w]*(?=\()/y },
];
...
function highlightWithRules(line: string, lineIndex: number, rules: CodeRule[]) {
  ...
  while (position < line.length) {
    ...
    nodes.push(<span className={match.rule.className} key={`${lineIndex}-${position}`}>{match.text}</span>);
