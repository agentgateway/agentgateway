// Vendored from modelcontextprotocol/typescript-sdk (MIT license):
//   test/conformance/src/everythingServer.ts @ ddba550fa9
//
// A v2-SDK (2026-07-28) MCP server that passes the conformance suite and bundles
// the fixtures the reference v1 everything-server lacks: the SEP-2243
// `test_x_mcp_header` tool and the SEP-2575 diagnostic tools. Used as an alternate
// conformance upstream so the full suite runs against a current, self-controlled v2
// server. See README.md for how it is wired into the harness.
//
// Kept byte-for-byte below this header, except the leading `#!/usr/bin/env node`
// shebang was dropped (unused; run via `tsx`). Re-vendor with:
//   gh api repos/modelcontextprotocol/typescript-sdk/contents/test/conformance/src/everythingServer.ts \
//     --jq '.content' | base64 -d
