...
impl Csrf {
	pub fn apply(&self, req: &mut Request) -> Result<PolicyResponse, filters::Error> {
		...
		let origin = get_origin_header(req)?;

		// 4. If no Origin header, allow
		if origin.is_none() {
			return Ok(Default::default());
		}
		...
		if origins_match(&origin_str, &target_origin) {
			return Ok(Default::default());
		}
	...
	fn is_request_exempt(&self, req: &Request) -> Result<bool, filters::Error> {
		if let Some(origin) = get_origin_header(req)? {
			for allowed_origin in &self.additional_origins {
...
fn get_target_origin(req: &Request) -> Result<String, filters::Error> {
	// After URI normalization, check authority first
	...
	let host_value = req
		.headers()
		.get(header::HOST)
		.ok_or_else(|| filters::Error::InvalidFilterConfiguration("Missing Host header".to_string()))?;
	...
	let scheme = req.uri().scheme_str().unwrap_or("http");
	Ok(format!("{}://{}", scheme, host_str))
}

/// Check if two origins match
fn origins_match(origin: &str, target_origin: &str) -> bool {
	// Parse both origins
	let origin_uri = match origin.parse::<::http::Uri>() {
		Ok(uri) => uri,
		Err(_) => return false,
	};

	let target_uri = match target_origin.parse::<::http::Uri>() {
		Ok(uri) => uri,
		Err(_) => return false,
	};

	// Compare authority (host:port)
	origin_uri.authority() == target_uri.authority()
}
