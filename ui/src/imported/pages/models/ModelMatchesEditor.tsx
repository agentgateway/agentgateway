  ...
  return (
    ...
    >
      ...
      <div className="button-row">
        <button className="button" type="button" onClick={() => props.onChange([...matches, { headers: [{ name: "", value: { exact: "" } }] }])}>
          <Plus size={16} />
          Add match
        </button>
      </div>
...
function MatchCard(props: {
  ...
  function updateHeader(index: number, header: HeaderMatch) {
    props.onChange({ ...props.match, headers: headers.map((item, itemIndex) => itemIndex === index ? header : item) });
  }

  function removeHeader(index: number) {
    props.onChange({ ...props.match, headers: headers.filter((_, itemIndex) => itemIndex !== index) });
  }
    ...
    <section className="match-card">
      <div className="match-card-header">
        ...
        <Tooltip content="Remove match">
          <button className="icon-button danger" type="button" aria-label={`Remove match ${props.index + 1}`} onClick={props.onRemove}>
            <Trash2 size={15} />
          </button>
        </Tooltip>
      </div>
      <div className="match-card-body">
        ...
        <button className="button small" type="button" onClick={() => props.onChange({ ...props.match, headers: [...headers, { name: "", value: { exact: "" } }] })}>
          <Plus size={16} />
          Add header
        </button>
      </div>
