...
impl Store {
		...
		match rule {
			...
			TrafficPolicy::OAuth2(p) => {
				...
				if let Some(compiled) = compiled {
					...
					warn!("ignoring oauth2 policy for route because it failed to compile");
				}
			},
	...
	fn inline_oauth2_cache_key(policy: &agent::OAuth2Policy) -> u64 {
		...
		// Potential collisions are guarded by `matches_policy` before cache reuse.
		let mut hasher = std::collections::hash_map::DefaultHasher::new();
		use std::hash::{Hash, Hasher};
		policy.issuer.hash(&mut hasher);
		policy.client_id.hash(&mut hasher);
		policy.client_secret.expose_secret().hash(&mut hasher);
...
impl StoreUpdater {
	...
	pub fn sync_local(
		...
		for p in &policies {
			s.compile_oauth2_filter_for_policy(p)?;
		}
