...
test('every page avoids word-by-word mixed Chinese copy', async ({ page }, testInfo) => {
	...
	await expect(page.getByRole('heading', { name: 'JWT 身份验证' })).toBeVisible();
	findings.push(
...
async function inspectPage(
	...
	await expect.poll(() => page.locator('body').innerText()).not.toMatch(/正在加载/);
	findings.push(...(await mixedCopyFindings(page)).map(text => ({ path, text })));
	if (process.env.I18N_SCREENSHOT) {
...
async function inspectEnglishPage(
	...
	await expect(page.locator('main.content')).toBeVisible();
	findings.push(...(await visibleChineseCopy(page)).map(text => ({ path, text })));
}
...
async function mixedCopyFindings(page: Page) {
	const bodyText = await page.locator('body').innerText();
	return [...new Set(bodyText.split('\n').map(line => line.trim()))].filter(isSuspiciousMixedCopy);
}
