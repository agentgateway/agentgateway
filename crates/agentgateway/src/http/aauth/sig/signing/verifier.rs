...
use crate::http::aauth::sig::digest::{calculate_content_digest, parse_content_digest_header};
use crate::http::aauth::sig::errors::Error;
use crate::http::aauth::sig::headers::{
	SignatureKey, parse_signature, parse_signature_input, parse_signature_key,
};
use crate::http::aauth::sig::keys::ed25519::{PublicKey, public_key_from_bytes, verify};
#[cfg(test)]
use crate::http::aauth::sig::signing::signature_base::build_signature_base;
use crate::http::aauth::sig::signing::signature_base::build_signature_base_raw;
...
pub fn verify_signature(
	...
	Ok(VerificationResult { scheme, agent_id })
}
...
mod tests {
	...
	use super::*;
	use crate::http::aauth::sig::keys::ed25519::{generate_keypair, public_key_to_base64url, sign};
	...
	fn test_round_trip_hwk_signature() {
		...
		let base = build_signature_base(
			...
			&crate::http::aauth::sig::headers::SignatureParams {
				created,
				keyid: None,
				nonce: None,
				alg: None,
		...
		let signature_value =
			crate::http::aauth::sig::headers::build_signature("sig1", &signature_bytes);
	...
	fn test_covered_components_match_case_insensitively() {
		...
		let params = crate::http::aauth::sig::headers::SignatureParams {
			created,
			keyid: None,
			nonce: None,
			alg: None,
		...
		.unwrap();
		let signature_bytes =
			crate::http::aauth::sig::keys::ed25519::sign(base.as_bytes(), &private_key);
		headers.insert(
			"Signature-Input".to_string(),
			crate::http::aauth::sig::headers::build_signature_input("sig1", &components, &params),
		);
		headers.insert(
			"Signature".to_string(),
			crate::http::aauth::sig::headers::build_signature("sig1", &signature_bytes),
