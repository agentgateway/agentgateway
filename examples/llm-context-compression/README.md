...
`anthropic-version`, `anthropic-beta`, `openai-beta`, and `cache-control` — so engines that
decide compressibility from headers stay cache-safe; credentials are never forwarded. Setting
`forwardHeaderMatches` **replaces** that default (it is not additive) — list the cache headers
yourself if you still need them, or use a matcher that matches nothing to forward no headers
at all.
