...
// SEP-2243: the gateway is an intermediary, so an inbound Mcp-Param-* it does not recognize MUST be
// forwarded to the upstream unchanged (header/body validation is the server's job, not the proxy's).
// Guards against re-introducing a strip/"leak guard".
#[tokio::test]
async fn inbound_custom_param_header_forwarded_upstream() {
	...
	let response = mcp_json_post(&client, &url, &body)
		...
		.await
		.unwrap();
	assert!(response.status().is_success(), "call should succeed");
	...
	assert!(
		captured
			.iter()
			.any(|h| h.get("mcp-param-region").and_then(|v| v.to_str().ok()) == Some("us-west1")),
		"inbound Mcp-Param-* must be forwarded upstream unchanged (SEP intermediary MUST-forward)"
	);
}
