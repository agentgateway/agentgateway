...
#[test]
fn test_opus_48_adaptive_thinking_and_output_config_are_preserved() {
	let provider = provider(None);
	let req = messages::typed::Request {
		model: "us.anthropic.claude-opus-4-8-v1".to_string(),
		messages: vec![messages::typed::Message {
			role: messages::typed::Role::User,
			content: vec![messages::typed::ContentBlock::Text(
				messages::typed::ContentTextBlock {
...
#[test]
fn test_opus_48_enabled_thinking_maps_to_adaptive_thinking() {
	let provider = provider(None);
	let req = messages::typed::Request {
		model: "us.anthropic.claude-opus-4-8-v1".to_string(),
		messages: vec![messages::typed::Message {
	...
	let inference = out.inference_config.unwrap();
	assert_eq!(inference.temperature, Some(0.4));
	assert_eq!(inference.top_p, Some(0.9));
	assert_eq!(inference.top_k, Some(20));
}
...
#[test]
fn test_completions_reasoning_effort_maps_to_adaptive_thinking_for_opus_48() {
	let provider = provider(None);
	let req: types::completions::Request = serde_json::from_value(json!({
		"model": "us.anthropic.claude-opus-4-8-v1",
		"messages": [
			{ "role": "user", "content": "Deeply analyze this topic." }
		],
	...
	let translated = super::from_completions::translate(&req, &provider, None, None).unwrap();
	...
	assert_ne!(
		translated["additionalModelRequestFields"]["thinking"]["type"],
		"enabled"
	);
	assert!(translated["additionalModelRequestFields"]["thinking"]["budget_tokens"].is_null());
}

#[test]
fn test_completions_reasoning_effort_uses_provider_model_for_opus_48_detection() {
	let provider = provider(Some("us.anthropic.claude-opus-4-8-v1"));
	let req: types::completions::Request = serde_json::from_value(json!({
		"model": "gpt-5",
		"messages": [
			{ "role": "user", "content": "Deeply analyze this topic." }
...
#[test]
fn test_opus_48_detection_accepts_1m_postfix() {
	let provider = provider(None);
	let req: types::completions::Request = serde_json::from_value(json!({
...
#[test]
fn test_messages_to_openai_compatible_to_bedrock_opus_48_preserves_adaptive_thinking() {
	let provider = provider(None);
	let messages_req: types::messages::Request = serde_json::from_value(json!({
	...
	let intermediate = crate::llm::conversion::completions::from_messages::translate(&messages_req)
		.expect("messages to completions translation");
	let completions_req: types::completions::Request =
		serde_json::from_slice(&intermediate).expect("valid completions request");
	let translated =
		super::from_completions::translate(&completions_req, &provider, None, None).unwrap();
	let translated: serde_json::Value = serde_json::from_slice(&translated).unwrap();
...
#[test]
fn test_responses_reasoning_effort_maps_to_adaptive_thinking_for_opus_48() {
	let provider = provider(None);
	let req: types::responses::Request = serde_json::from_value(json!({
		"model": "us.anthropic.claude-opus-4-8-v1",
		"max_output_tokens": 64,
