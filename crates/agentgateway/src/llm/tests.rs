...
#[test]
fn test_process_error_messages_uses_completions_translation_for_provider_adapters() {
	let openai_body = provider_error_json();
	let expected = conversion::completions::from_messages::translate_error(&openai_body)
		.expect("expected error translation should succeed");
	let expected: Value = serde_json::from_slice(&expected).expect("expected error should be JSON");

	// Google providers return errors in a different schema (and array-wrapped).
	// See: https://github.com/openai/openai-node/issues/1734
	let google_body = Bytes::from(
		serde_json::json!([{
			"error": {
				"code": 400,
				"message": "bad request payload",
	...
	for (name, provider) in provider_message_adapters() {
		let req = llm_request(InputFormat::Messages, name, "gpt-4.1", false);
		let body = match name {
			"gemini" | "vertex" => &google_body,
			_ => &openai_body,
		};
		let got = provider
			.process_error(&req, body)
			.unwrap_or_else(|e| panic!("{name}: process_error failed: {e}"));
		let got: Value = serde_json::from_slice(&got)
			.unwrap_or_else(|e| panic!("{name}: invalid translated error: {e}"));
		// All providers should produce the same Messages error shape, regardless of input format
		assert_eq!(got, expected, "{name}: translated error mismatch");
	}
}
