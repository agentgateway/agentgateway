...
use crate::http::aauth::sig::headers::{SignatureKey, parse_signature_key};
use crate::http::aauth::sig::keys::ed25519::PublicKey;
use crate::http::aauth::sig::keys::jwk::Jwk;
use crate::http::aauth::sig::keys::jwk_thumbprint::calculate_jwk_thumbprint;
use crate::http::aauth::sig::signing::{SignatureScheme, resolve_hwk_public_key, verify_signature};
use crate::http::aauth::tokens::{
	AAuthError, decode_jwt_claims_unverified, decode_jwt_header, get_string_claim,
	validate_agent_token, validate_auth_token,
};
...
use crate::{apply, *}; // brings in apply, JsonSchema (under `schema` feature), schema_de!, schema_enum!, etc.
...
impl JwksCache {
	pub fn get(&self, id: &str, dwk: &str, kid: &str) -> Option<Jwk> {
		...
		// Fast path: warm cache hit.
		{
			...
			// Absent entry: nothing cached and nothing to evict, so there is no slow path to take.
...
impl AAuth {
	async fn apply_inner(&self, req: &mut Request, client: &PolicyClient) -> Result<(), Error> {
		...
		let resolver =
			move |sig_key: &SignatureKey| -> Result<PublicKey, crate::http::aauth::sig::Error> {
				match sig_key.scheme.as_str() {
					"hwk" => resolve_hwk_public_key(sig_key),
					"jwks_uri" => jwks_key_for_resolver.ok_or_else(|| {
						crate::http::aauth::sig::Error::InvalidKey("jwks_uri key not pre-resolved".to_string())
					}),
					"jwt" => jwt_key_for_resolver.ok_or_else(|| {
						crate::http::aauth::sig::Error::InvalidKey("jwt key not pre-resolved".to_string())
					}),
					other => Err(crate::http::aauth::sig::Error::UnsupportedScheme(
						other.to_string(),
					)),
				}
			};
...
/// Translate an `crate::http::aauth::sig::Error` into the policy-level [`Error`] enum, preserving any
/// required-component hints so the response body can name the missing components.
fn map_signing_error(error: crate::http::aauth::sig::Error) -> Error {
	use crate::http::aauth::sig::Error as E;
	match &error {
