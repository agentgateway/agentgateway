...
export function corsNeedsUpdate(cors: CorsPolicy | null | undefined, target: CorsTarget, origin = currentOrigin()) {
  if (!cors) return true;
  return !hasValue(cors.allowOrigins, origin) ||
...
export function applyPlaygroundCors(config: GatewayConfig, target: CorsTarget, origin = currentOrigin()) {
  if (target === "llm") {
    config.llm ??= { models: [] };
    config.llm.policies ??= {};
    config.llm.policies.cors = withPlaygroundCors(config.llm.policies.cors, target, origin);
    return;
  }

  config.mcp ??= { targets: [] };
  config.mcp.policies ??= {};
  config.mcp.policies.cors = withPlaygroundCors(config.mcp.policies.cors, target, origin);
}

function withPlaygroundCors(cors: CorsPolicy | null | undefined, target: CorsTarget, origin: string): CorsPolicy {
  return {
    ...(cors ?? {}),
    ...
    exposeHeaders: target === "mcp" ? appendUnique(cors?.exposeHeaders, "Mcp-Session-Id") : cors?.exposeHeaders,
  };
}
