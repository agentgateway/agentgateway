...
impl StreamableHttpService {
	...
	pub async fn handle(&self, request: Request, inputs: RelayInputs) -> Result<Response, ProxyError> {
	...
	pub async fn handle_post(&self, request: Request, inputs: RelayInputs) -> Result<Response, ProxyError> {
		// check accept header
		if !request
			.headers()
	...
	pub async fn handle_get(&self, request: Request, inputs: RelayInputs) -> Result<Response, ProxyError> {
		// check accept header
		if !request
			.headers()
			.get(http::header::ACCEPT)
