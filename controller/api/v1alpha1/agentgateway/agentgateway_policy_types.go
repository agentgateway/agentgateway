...
// +kubebuilder:validation:AtMostOneOf=key;secretRef;passthrough;aws;azure;gcp
// +kubebuilder:validation:XValidation:rule="has(self.headers) || has(self.key) || has(self.secretRef) || has(self.passthrough) || has(self.aws) || has(self.azure) || has(self.gcp)",message="must specify headers, or at most one of key/secretRef/passthrough/aws/azure/gcp (headers may be combined with one of the primary auth kinds)"
